...
def _minify(basedir, factors=[], resolutions=[]):
    ...
    imgdir = sorted(glob.glob(os.path.join(basedir, 'cam*')))
    imgs = [os.path.join(f, os.listdir(f)[0]) for f in sorted(glob.glob(os.path.join(basedir, 'cam*')))]
    ...
    for r in factors + resolutions:
        ...
        os.makedirs(imgdir)
        check_output('cp {}/* {}'.format(imgdir_orig[0], imgdir), shell=True)
...
def _load_data(basedir, factor=None, width=None, height=None, load_imgs=True, load_depths=False):
    ...
    imgfiles = [os.path.join(f, os.listdir(f)[0]) for f in sorted(glob.glob(os.path.join(basedir, 'cam*')))]
...
def load_facebook_data(basedir, factor=8, width=None, height=None,
