...
def compute_bbox_by_cam_frustrm(args, cfg, HW, Ks, poses, i_train, near, far, **kwargs):
    ...
    if cfg.data.unbounded_inward:   #no
...
def scene_rep_reconstruction(args, cfg, cfg_model, cfg_train, xyz_min, xyz_max, data_dict, stage, coarse_ckpt_path=None):
    ...
    if args.no_reload:   #no
        reload_ckpt_path = None
    elif args.ft_path:   #no
        reload_ckpt_path = args.ft_path
    elif os.path.isfile(last_ckpt_path):   #no
        reload_ckpt_path = last_ckpt_path
    else:    #yes
        reload_ckpt_path = None

    # init model and optimizer
    if reload_ckpt_path is None:     #yes   没有预训练的模型
    ...
    def gather_training_rays():
        if data_dict['irregular_shape']:      #把img放在gpu上面
            ...
            #[17, 3024, 4032, 3]  训练的图片
        if cfg_train.ray_sampler == 'in_maskcache':   #no
        ...
        elif cfg_train.ray_sampler == 'flatten':    #yes
    ...
    if cfg_train.maskout_lt_nviews > 0:   #no
    ...
    for global_step in trange(1+start, 1+cfg_train.N_iters):   #0 30000
        ...
        if cfg.data.load2gpu_on_the_fly:   #no
        ...
        if cfg_train.weight_nearclip > 0:   #no
        ...
        if cfg_train.weight_rgbper > 0:   #yes
...
def train(args, cfg, data_dict):
    ...
    eps_fine = time.time()
    if cfg.coarse_train.N_iters == 0:   #yes
        ...
        if args.render_test or args.render_train or args.render_video:
            ...
            render_viewpoints_kwargs = {
                ...
                'render_kwargs': {
                    'near': data_dict['near'],
                    'far': data_dict['far'],
                    'bg': 1 if cfg.data.white_bkgd else 0,   #0
                    'stepsize': stepsize,
                    'inverse_y': cfg.data.inverse_y,
                    'flip_x': cfg.data.flip_x,
                    'flip_y': cfg.data.flip_y,
